...
test("product detail uses a summary-first responsive layout", async () => {
  ...
  assert.match(source, /sections\.warrantyPolicy/);
  assert.doesNotMatch(source, /<CardTitle>\{title\}<\/CardTitle>/);
});

test("product detail exposes copy actions", async () => {
  const source = await readFile(detailCardUrl, "utf8");
...
test("product summary and policy sections preserve mobile width", async () => {
  const source = await readFile(detailCardUrl, "utf8");
  const summarySource = source.slice(
    source.indexOf("function ProductSummaryHeader"),
    source.indexOf("export function ProductDetailSkeleton"),
  );
  ...
  assert.match(source, /title=\{t\("sections\.warrantyPolicy"\)\}/);
  assert.match(
    source,
    /<CopyableDetailItem\s+icon=\{<Hash className="size-4" \/>\}\s+label=\{t\("productCode"\)\}/,
  );
  assert.doesNotMatch(source, /product\.owner/);
  assert.doesNotMatch(source, /product\.warranty\?\.status/);
});
