...
@Injectable()
export class GetVietnamProvinceUseCase {
  constructor(
    private readonly provincesClient: VietnamProvincesClient,
    private readonly cacheService: LocationsCacheService,
  ) {}

  execute(code: number, depth = 1) {
    return this.cacheService.remember(
      `locations:vietnam:province:${code}:depth=${depth}`,
      () => this.provincesClient.getProvince(code, depth),
    );
  }
}
