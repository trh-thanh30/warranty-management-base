...
test("overlay maps require an intentional click before capturing gestures", async () => {
  const source = await readFile(sharedMapPath, "utf8");

  assert.match(source, /scrollWheelZoom=\{activationMode === "direct"\}/);
  assert.match(source, /dragging=\{activationMode === "direct"\}/);
  assert.match(source, /touchZoom=\{activationMode === "direct"\}/);
  assert.match(source, /touchAction:[\s\S]*"pan-y"/);
  assert.match(source, /useMapEvents\(\{\s*click:/);
  assert.match(source, /setMapInteractionEnabled\(map, true\)/);
  assert.match(source, /mouseleave/);
  assert.match(source, /event\.key !== "Escape"/);
  assert.match(source, /setMapInteractionEnabled\(map, false\)/);
  assert.match(source, /activationMode === "overlay"\s*\?\s*\(/);
});

test("shared map exposes accessible controls for entering and leaving map interaction", async () => {
  const source = await readFile(sharedMapPath, "utf8");
  ...
  assert.match(source, /aria-label=\{deactivateLabel\}/);
  assert.match(source, /<X/);
  assert.match(source, /min-h-11/);
  assert.match(source, /isInteractionEnabled\s*\?\s*"[^"]*opacity-0/);
  ...
  assert.match(source, /onInteractionChange=\{setIsInteractionEnabled\}/);
});
...
test("about map composes feature layers inside the shared map", async () => {
  ...
  assert.match(source, /deactivateLabel=\{t\("deactivateMap"\)\}/);
